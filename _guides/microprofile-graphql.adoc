...
Preventing Over-fetching and Under-fetching::
    REST API's are server-driven fixed data responses that can not be determined by
...
First, create a the following entities representing a film from a galaxy far far away:
...
The Quarkus `smallrye-graphql` extension comes with `GraphiQL` that by
default is started in `dev` and `test` modes and by configuration in `production`.
...
Therefore, the name of the query is implicitly set as the function name
excluding the `get`.
...
will be `Skywalker` when the parameter is not given.
